  ...
  if (group_ > 1) {
    if (group_ == input[0].get_shape()[1] &&
      ...
      switch (input[0].get_type()) {
        case Type::kFloat:
          DepthwiseConv4DRelu<float>(input[0], *kernel_, *bias_, output[0],
                                     stride_, pads_, dilations_, backend);
          break;
        case Type::kInt:
          DepthwiseConv4DRelu<int>(input[0], *kernel_, *bias_, output[0],
                                   stride_, pads_, dilations_, backend);
          break;
        default:
  ...
  switch (input[0].get_type()) {
    case Type::kInt: {
      if (kernel_->get_shape().dims() == 2) {
        ...
        Conv4DRelu<int>(input[0], *kernel_, *bias_, output[0], stride_, pads_,
    ...
    case Type::kFloat: {
      if (kernel_->get_shape().dims() == 2) {
        ...
        if (useLegacyImpl_) {
          Conv4D_LegacyRelu<float>(input[0], *kernel_, *bias_, output[0],
                                   stride_, pads_, dilations_, backend);
        } else {
          Conv4DRelu<float>(input[0], *kernel_, *bias_, output[0], stride_,
                            pads_, group_, dilations_, backend);
        }
      }
      break;
    }
